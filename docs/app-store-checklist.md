...
**最近更新：2026-09-15（1.1.0 过审上架）**

> 当前状态：版本 `1.1.0`（构建 `1.1.0 (7)`，2026-09-15）与 `1.0`（构建 `1.0.0 (5)`）均 READY_FOR_SALE；中国区在售。2026-09-03 已创建供应范围并仅启用中国大陆（`CHN`）；App Store Connect 回读为 `AVAILABLE`，中国区公开商店查询已返回 Quite Note，价格为免费。其余 174 个地区保持关闭，新增地区不会自动开放。

> 🎉 **2026-09-15 1.1.0 当日过审上架（READY_FOR_SALE）**：早 5:08 上传构建 `1.1.0 (7)` → 中午 ASC 网页自动化提交审核（必填项"此版本的新增内容"补齐）→ 下午撤回审核补传 5 张新营销截图（豆包 AI 生成/PIL 合成，2560×1600，合计 8/10 张）→ 傍晚重新提交 → 晚间过审，中国区商店已展示 1.1.0。发布说明为中性文案（"优化了整体使用体验…"），下版本建议改具体。
> 🔧 **同日 ASC API 工具固化为常驻脚本 `tools/asc-api.py`**（ES256 JWT 零依赖；`versions`/`shots`/`get` 三命令；凭据默认值内置，Issuer 完整值见桌面《5-CI-CD配置指南.md》——旧 `/tmp/asc/api.py` 已随重启丢失，勿再引用）。状态查询一条命令替代整个浏览器流程，本次过审结论即由 API 确认。

> 🧪 **2026-09-13 本地预检（未上传、未提交）**：已生成 App Store 构建 `1.3.3 (8)`，并在编译期移除了启动器中的系统命令、全目录文件检索和终止其他 App 功能；225 项测试、签名、pkg 和最终二进制审计均通过。下一次实际提审前仍需按最终版本做一次干净环境冒烟测试。
...
**Quite Note 1.1.0 已在中国大陆 App Store 免费上架（2026-09-15，当日提审当日过审）。**
